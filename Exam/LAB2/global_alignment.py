...
def recursive_formula(sim_upleft, sim_left, sim_up, sub_score, gap_score):
    ...
    if optimal_move == 0:
        ...
        return 1, optimal_score

    return 2, optimal_score


def backtrack(query, reference, best_moves):
    ...
    i, j = best_moves.shape
    i -= 1
    j -= 1

    while best_moves[i, j] != -1:
...
def path_graph(query, reference, scores):
    ...
    backtrack(query, reference, best_moves)
